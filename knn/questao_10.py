...
base_dados = []
with open('./recomendacao/baseTransacaoRecomendacao.csv', newline='') as csvFile:
    reader = csv.reader(csvFile, delimiter=',')
    _ = next(reader)
    
    base_dados = [[float(row[1]), int(row[2]), int(row[3]), 
                   int(row[4]), int(row[5]), row[6]] for row in reader if row]


base_normalizada_zscore, closures_zscore = normalizar_base(5, base_dados, z_score)
base_normalizada_minmax, closures_minmax = normalizar_base(5, base_dados, min_max)


descobrir = [4.5, 1, 1, 0, 0]
